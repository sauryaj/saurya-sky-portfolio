...
const GridTile = (props: GridTileProps) => {
  ...
  const { title, textAlign, children, color, position, id, scale = 1 } = props;
  ...
  useEffect(() => {
    if (isMobile && titleRef.current) {
      ...
      gsap.to(titleRef.current.position, { x: isWork ? 1 : id === 'certifications' ? 0 : -1, y: isWork ? -1.7 : id === 'certifications' ? -2.05 : 1.5, duration: 0.5 });
    }
  }, [id]);

  useFrame(() => {
    const d = data.range(0.95, 0.05);
    if (isMobile && titleRef.current) (titleRef.current as any).fillOpacity = d;
  });

  const handleEscape = (e: KeyboardEvent) => { if (e.key === 'Escape') exitPortal(true); };
  const portalInto = (e: React.MouseEvent) => {
    ...
    if (!document.querySelector('.close')) {
      ...
      gsap.fromTo(div, { scale: 0, rotate: '-180deg' }, { opacity: 1, zIndex: 10, transform: 'rotateX(0deg)', scale: 1, duration: 1 });
    }
  ...
  const exitPortal = (force = false) => {
    ...
    gsap.to(document.querySelector('.close'), { scale: 0, duration: 0.5, onComplete: () => document.querySelectorAll('.close').forEach((el) => el.remove()) });
    document.body.removeEventListener('keydown', handleEscape);
  };
  const fontProps: Partial<TextProps> = { font: "./soria-font.ttf", maxWidth: 2, anchorX: 'center', anchorY: 'bottom', fontSize: 0.7, color: 'white', textAlign, fillOpacity: 0 };
  const onPointerOver = () => {
  ...
  const getGeometry = () => {
    if (!isMobile) return <planeGeometry args={[4, 4, 1]} />;
    const isWork = id === 'work';
    const isCertifications = id === 'certifications';
    const points = isWork ? [[-1, 2, 0], [-1, -2, 0], [3, -2, 0]] : isCertifications ? [[-2, 1.8, 0], [2, 1.8, 0], [0, -2, 0]] : [[-3, 2, 0], [1, -2, 0], [1, 2, 0]];
    return <primitive object={TriangleGeometry({ points })} attach="geometry" />;
  };

  return (
    <mesh ref={gridRef} position={position} scale={scale} onClick={portalInto} onPointerOver={onPointerOver} onPointerOut={onPointerOut}>
      {getGeometry()}
      <group>
